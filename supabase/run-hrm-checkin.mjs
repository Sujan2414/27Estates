...
const sql = readFileSync(join(__dirname, 'hrm-checkin.sql'), 'utf8')
...
for (const stmt of statements) {
    ...
    if (!res.ok && !data.message?.includes('already exists') && !data.message?.includes('does not exist') && !data.message?.includes('duplicate column')) {
        console.warn(`⚠  ${data.message} — ${query.slice(0, 80).replace(/\n/g, ' ')}`); failed++
    } else {
