...
// Identify the requesting user via the SSR cookie client and look up their
// role. Returns { id, role } or null if not signed in. The service-role
// `getSupabase` we use for the actual data queries doesn't carry user
// context, so we have to bring it in separately and apply role-based
// filters ourselves.
async function getRequestUser(): Promise<{ id: string; role: string | null } | null> {
    try {
...
export async function GET(request: NextRequest) {
    ...
    // Role-based scope. Admins/super_admins see everything; agents see
    // only visits assigned to them. Anonymous requests return empty.
    const me = await getRequestUser()
    ...
    if (!isAdmin) query = query.eq('agent_id', me.id)
