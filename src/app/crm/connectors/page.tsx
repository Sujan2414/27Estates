...
const platformInfo: Record<string, { label: string; color: string; description: string }> = {
    ...
    website: { label: 'Website', color: 'var(--crm-accent)', description: 'Direct website inquiries' },
    ...
    return (
        <div className={styles.pageContent}>
            ...
            <div className={styles.card} style={{ marginBottom: '1.5rem', borderLeft: '3px solid #BFA270' }}>
                <h3 style={{ fontSize: '0.875rem', fontWeight: 600, color: 'var(--crm-accent)', marginBottom: '0.5rem' }}>How Webhooks Work</h3>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(380px, 1fr))', gap: '1rem' }}>
                {connectors.map(connector => {
                    const info = platformInfo[connector.platform] || {
                        label: connector.platform, color: 'var(--crm-text-faint)',
                        ...
                        <div key={connector.id} className={styles.card} style={{ padding: 0, overflow: 'hidden' }}>
                            ...
                            <div style={{
                                ...
                                <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
                                    ...
                                    <div>
                                        <div style={{ fontSize: '0.875rem', fontWeight: 600, color: 'var(--crm-text-secondary)' }}>{info.label}</div>
                                        <div style={{ fontSize: '0.6875rem', color: 'var(--crm-text-faint)' }}>{info.description}</div>
                                    </div>
                                </div>
                                ...
                                <button
                                    ...
                                    style={{
                                        ...
                                        backgroundColor: connector.is_active ? '#22c55e' : 'var(--crm-border-subtle)',
                            ...
                            <div style={{ padding: '0.75rem 1rem', display: 'flex', gap: '1.5rem' }}>
                                <div>
                                    <div style={{ fontSize: '1.125rem', fontWeight: 700, color: 'var(--crm-text-secondary)' }}>{connector.leads_received}</div>
                                    <div style={{ fontSize: '0.6875rem', color: 'var(--crm-text-faint)' }}>Leads Received</div>
                                </div>
                                <div>
                                    <div style={{ fontSize: '0.8125rem', fontWeight: 500, color: 'var(--crm-text-muted)' }}>{formatDate(connector.last_webhook_at)}</div>
                                    <div style={{ fontSize: '0.6875rem', color: 'var(--crm-text-faint)' }}>Last Webhook</div>
                                </div>
                            </div>

                            {/* Webhook URL */}
                            <div style={{ padding: '0.75rem 1rem', borderTop: '1px solid var(--crm-border)' }}>
                                <div style={{ fontSize: '0.6875rem', color: 'var(--crm-text-faint)', marginBottom: '0.375rem' }}>Webhook URL</div>
                                <div style={{
                                    display: 'flex', alignItems: 'center', gap: '0.5rem',
                                    backgroundColor: 'var(--crm-bg)', borderRadius: '0.375rem',
                                    padding: '0.5rem 0.75rem', border: '1px solid var(--crm-border)',
                                }}>
                                    <code style={{ flex: 1, fontSize: '0.6875rem', color: 'var(--crm-text-muted)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                            ...
                            {(connector.platform === 'meta_ads' || connector.platform === 'google_ads') && (
                                <div style={{ padding: '0.75rem 1rem', borderTop: '1px solid var(--crm-border)' }}>
                                    <div style={{ fontSize: '0.6875rem', color: 'var(--crm-text-faint)', marginBottom: '0.375rem' }}>Setup</div>
                                    {connector.platform === 'meta_ads' && (
                                        <ol style={{ fontSize: '0.6875rem', color: 'var(--crm-text-muted)', paddingLeft: '1rem', margin: 0, lineHeight: 1.8 }}>
                                            <li>Go to Meta Business Suite &gt; Integrations &gt; Leads Access</li>
                                            <li>Add webhook URL above under &quot;Webhooks&quot;</li>
                                            <li>Set verify token: <code style={{ color: 'var(--crm-accent)' }}>META_WEBHOOK_VERIFY_TOKEN</code> env var</li>
                                            <li>Subscribe to <code style={{ color: 'var(--crm-accent)' }}>leadgen</code> field</li>
                                        </ol>
                                    )}
                            ...
                            {(connector.platform === '99acres' || connector.platform === 'magicbricks' || connector.platform === 'housing' || connector.platform === 'justdial') && (
                                <div style={{ padding: '0.75rem 1rem', borderTop: '1px solid var(--crm-border)' }}>
                                    <div style={{ fontSize: '0.6875rem', color: 'var(--crm-text-muted)', lineHeight: 1.6 }}>
            ...
            {connectors.length === 0 && (
                <div className={styles.card}>
                    <div className={styles.emptyState}>
                        <Plug size={32} style={{ color: 'var(--crm-text-dim)', marginBottom: '0.5rem' }} />
                        <p style={{ color: 'var(--crm-text-secondary)', fontWeight: 500, marginBottom: '0.5rem' }}>No connectors found</p>
