    ...
    return (
        <div className={styles.dashboard}>
            ...
            <div className={propertyStyles.searchBar}>
                ...
                <input type="text" placeholder="Search warehouse listings..." value={searchQuery}
            ...
            <div style={{ display: 'flex', alignItems: 'flex-start', gap: '10px', padding: '12px 16px', marginBottom: '1.25rem', backgroundColor: '#fefce8', border: '1px solid #fde68a', borderRadius: '10px' }}>
                ...
                <div style={{ fontSize: '0.8125rem', color: '#92400e', lineHeight: 1.5 }}>
                    <strong>Featured Priority (1–6):</strong> Click the ☆ star on any card to feature it on the website. The next available slot (1→6) is auto-assigned. Featured listings appear first in the Warehouse section in that order. Click a filled star to remove it.
                </div>
            </div>
            ...
            {showBulkModal && (
                <BulkUploadModal type="project" section="warehouse" onClose={() => setShowBulkModal(false)} onComplete={() => { fetchProjects() }} />
            )}
        </div>
    )
}
