...
export default function NewCareerOpeningPage() {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        const { error } = await supabase.from('career_openings').insert(payload);

        if (error) {
            console.error('Error creating opening:', error);
            alert('Failed to create opening. Please try again.');
            setSaving(false);
            return;
        }
        ...
        <div className={styles.dashboard}>
            <div className={styles.header}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
                    ...
                    <div>
                        <h1 className={styles.pageTitle}>New Career Opening</h1>
                        <p className={styles.pageSubtitle}>Create a new job listing</p>
                    </div>
                </div>
            </div>
