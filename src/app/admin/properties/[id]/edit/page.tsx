...
export default function EditPropertyPage() {
    ...
    const fetchProperty = async () => {
        ...
        setFormData({
            ...
            built_up_area: data.built_up_area?.toString() || '',
            carpet_area: data.carpet_area?.toString() || '',
            plot_size: data.plot_size?.toString() || '',
            ...
            // Commercial features (stored in commercial_details JSONB)
            workstation: data.commercial_details?.workstation?.toString() || '',
            cabin: data.commercial_details?.cabin?.toString() || '',
            conference_room: data.commercial_details?.conference_room?.toString() || '',
            reception_area: data.commercial_details?.reception_area?.toString() || '',
            power_kva: data.commercial_details?.power_kva?.toString() || '',
            power_backup: data.commercial_details?.power_backup || '',
            // Warehouse features (stored in warehouse_details JSONB)
            pollution_zone: data.warehouse_details?.pollution_zone || 'Green',
            racking_capacity_tonnes: data.warehouse_details?.racking_capacity_tonnes?.toString() || '',
            floor_strength: data.warehouse_details?.floor_strength?.toString() || '',
            stp_etp_capacity: data.warehouse_details?.stp_etp_capacity?.toString() || '',
            loading_bays: data.warehouse_details?.loading_bays?.toString() || '',
            canopy_length: data.warehouse_details?.canopy_length?.toString() || '',
            canopy_width: data.warehouse_details?.canopy_width?.toString() || '',
            fire_noc: data.warehouse_details?.fire_noc || false,
            approval_plan: data.warehouse_details?.approval_plan || false,
            dock_levellers: data.warehouse_details?.dock_levellers || false,
        })
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const propertyData = {
                ...
                // Commercial features → JSONB
                commercial_details: (formData.workstation || formData.cabin || formData.conference_room || formData.power_kva) ? {
                ...
                // Warehouse features → JSONB
                warehouse_details: (formData.racking_capacity_tonnes || formData.loading_bays || formData.floor_strength || formData.fire_noc || formData.approval_plan || formData.dock_levellers) ? {
