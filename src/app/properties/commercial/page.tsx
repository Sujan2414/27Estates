...
import { Search as SearchIcon, ChevronDown, X, SlidersHorizontal, Landmark, Search, Building2, ShoppingBag, Users, Monitor, LayoutGrid, ShoppingCart, Briefcase } from "lucide-react";
...
const subCategoryOptions = [
    ...
    { id: "Retail Shops", label: "Retail Shops", icon: ShoppingBag },
    { id: "Co-Working Space", label: "Co-Working Space", icon: Users },
    { id: "Showroom", label: "Showroom", icon: Monitor },
    { id: "Mixed Use", label: "Mixed Use", icon: LayoutGrid },
    { id: "Business Park", label: "Business Park", icon: Briefcase },
...
const CommercialPage = () => {
    ...
    const [filteredProjects, setFilteredProjects] = useState<CommercialProject[]>([]);
    ...
    const [ocApprovedOnly, setOcApprovedOnly] = useState(false);
    ...
    const fetchProjects = async () => {
        ...
        if (!error && data) {
            ...
            setFilteredProjects(data as CommercialProject[]);
        }
        setLoading(false);
    };
    ...
    useEffect(() => {
        ...
        if (ocApprovedOnly) result = result.filter(p => p.is_oc_approved);
    ...
    }, [projects, searchQuery, selectedListingType, selectedSubCategory, selectedStatus, selectedCity, selectedArea, selectedDirection, minPrice, maxPrice, selectedDeveloper, reraApprovedOnly, ocApprovedOnly, featuredOnly, sortBy, pincodeSearch]);
    ...
    const handleReset = () => {
        ...
        setOcApprovedOnly(false);
        setFeaturedOnly(false);
        setSortBy("newest");
    };

    const hasActiveFilters = !!(selectedSubCategory || selectedStatus || selectedCity || selectedArea || minPrice || maxPrice || selectedDeveloper || reraApprovedOnly || ocApprovedOnly || featuredOnly);
    ...
    const FilterContent = () => (
        <>
            ...
            {/* OC Approved Toggle */}
            <div className={styles.toggleRow}>
                <span className={styles.toggleLabel}>OC Approved Only</span>
                <div className={styles.toggle}>
                    <button
                        className={`${styles.toggleButton} ${ocApprovedOnly ? styles.toggleButtonActive : ''}`}
                        onClick={() => setOcApprovedOnly(!ocApprovedOnly)}
                    >
                        <span className={`${styles.toggleIndicator} ${ocApprovedOnly ? styles.toggleIndicatorActive : ''}`} />
                    </button>
                </div>
            </div>

            {/* Sub-Category / Space Type */}
        ...
        return (
            <div className={styles.searchPage}>
                <div className={styles.loadingContainer}>
                    <div className={styles.loadingText}>Loading commercial listings...</div>
                </div>
            </div>
        );
    }
        ...
        <div className={styles.searchPage}>
            ...
            <aside className={styles.filterSidebar}>
                <div className={styles.filterHeader}>
                    <h1 className={styles.filterTitle}>Commercial Filters</h1>
                </div>
                ...
                <div className={styles.listingsScrollArea} data-lenis-prevent>
                    {filteredProjects.length > 0 ? (
                        <div className={styles.grid}>
                            {filteredProjects.map(project => (
                                <ProjectCard
                                    ...
                                    category={project.sub_category || 'Commercial'}
                        ...
                        <div className={styles.emptyState}>
                            <Landmark size={48} className={styles.emptyIcon} strokeWidth={1} />
                            <p className={styles.emptyText}>No commercial listings found</p>
                            <p className={styles.emptySubtext}>{hasActiveFilters ? 'Try adjusting your filters' : 'No commercial listings available yet'}</p>
                        </div>
                    )}
                </div>
            </main>
            ...
            <div className={showFilterModal ? styles.filterModalOpen : styles.filterModal}>
                <div className={styles.filterModalHeader}>
                    <h2 className={styles.filterModalTitle}>Commercial Filters</h2>
