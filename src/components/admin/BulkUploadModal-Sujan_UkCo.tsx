...
export default function BulkUploadModal({ type, onClose, onComplete }: BulkUploadModalProps) {
    ...
    const handleUpload = async () => {
        ...
        if (type === 'property') {
            ...
            // Sync unique developer names to the developers table
            if (done > 0) {
                const uniqueDevNames = new Set<string>()
                for (const p of parsedProjects) {
                    if (p.developer_name && p.developer_name.trim() !== '') {
                        uniqueDevNames.add(p.developer_name.trim())
                    }
                }
                ...
                for (const devName of uniqueDevNames) {
                    ...
                    if (!existingDevs || existingDevs.length === 0) {
                        await supabase.from('developers').insert([{ name: devName }])
                    }
                }
            }
        }
