...
const TestimonialCard = ({ testimonial }: { testimonial: Testimonial }) => (
    <div
        className="flex items-start gap-3 rounded-2xl backdrop-blur-xl border p-5 w-64"
        style={{
            backgroundColor: 'rgba(255, 255, 255, 0.9)',
            borderColor: 'rgba(31, 82, 75, 0.2)',
        }}
    >
        <img src={testimonial.avatarSrc} className="h-10 w-10 object-cover rounded-xl" alt="avatar" />
        <div className="text-sm leading-snug">
            <p className="flex items-center gap-1 font-medium" style={{ color: '#1a1a1a' }}>{testimonial.name}</p>
            <p style={{ color: '#666666' }}>{testimonial.handle}</p>
            <p className="mt-1" style={{ color: '#333333' }}>{testimonial.text}</p>
        </div>
    </div>
);
...
export const SignInPage: React.FC<SignInPageProps> = ({
    title = <span className="font-light tracking-tight">Welcome Back</span>,
